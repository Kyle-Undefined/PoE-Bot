namespace PoE.Bot.Helpers
{
    ...
    {
        public static async Task<Embed> WikiGetItemAsync(string item, HttpClient httpClient)
        {
            ...
            string jsonSearch = await httpClient.GetStringAsync(searchURL + WebUtility.UrlEncode(item)).ConfigureAwait(false);
            ...
            if (jsonObj.Count > 0)
            {
                ...
                string jsonParse = await httpClient.GetStringAsync(parseURL + wikiPage).ConfigureAwait(false);
                ...
                if (jsonParse.Contains("Redirect"))
                {
                    ...
                    jsonParse = await httpClient.GetStringAsync(parseURL + wikiPage).ConfigureAwait(false);
                }
                ...
                if (itemType is 2)
                {
                    ...
                    string jsonParseSections = await httpClient.GetStringAsync(sectionsURL + wikiPage).ConfigureAwait(false);
                    ...
                    {
                        string jsonSection = await httpClient.GetStringAsync(string.Format(parseSectionsURL, sectionNumber, wikiPage)).ConfigureAwait(false);
