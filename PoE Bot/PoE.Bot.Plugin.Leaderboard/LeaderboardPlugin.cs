...
namespace PoE.Bot.Plugin.Leaderboard
{
    public class LeaderboardPlugin : IPlugin
    {
        ...
        public Type ConfigType { get { return typeof(LeaderboardPluginConfig); } }
        public string Name { get { return "Leaderboard Plugin"; } }
        private Timer LeaderboardTimer { get; set; }
        private LeaderboardPluginConfig conf;

        public static LeaderboardPlugin Instance { get; private set; }

        public void Initialize()
        {
            Log.W(new LogMessage(LogSeverity.Info, "Leaderboard Plugin", "Initializing Leaderboard"));
            Instance = this;
            this.conf = new LeaderboardPluginConfig();
            this.LeaderboardTimer = new Timer(new TimerCallback(Leaderboard_Tick), null, 5000, 3600000);
            Log.W(new LogMessage(LogSeverity.Info, "Leaderboard Plugin", "Leaderboard Initialized"));
        }

        public void LoadConfig(IPluginConfig config)
        {
            var cfg = config as LeaderboardPluginConfig;
            if (cfg != null)
                this.conf = cfg;
        }
        ...
        public void RemoveLeaderboard(string variant, ulong channel)
        {
        ...
        internal IEnumerable<Leaderboard> GetLeaderboards(ulong[] channels)
        {
            foreach (var lb in this.conf.Leaderboards)
                if (channels.Contains(lb.ChannelId))
                    yield return lb;
        }

        private void UpdateConfig()
        {
            Log.W(new LogMessage(LogSeverity.Info, "Leaderboard Plugin", "Updating config"));
        ...
        private void Leaderboard_Tick(object _)
        {
                ...
                if (lb.Enabled)
                {
                    ...
                    if (racers.Count > 0)
                    {
                        ...
                        if (embedClasses.Fields.Count > 0)
                            PoE_Bot.Client.SendEmbed(embedClasses, lb.ChannelId);

                        if (embedAscendancy.Fields.Count > 0)
                            PoE_Bot.Client.SendEmbed(embedAscendancy, lb.ChannelId);

                        if (embedAscendancyCont.Fields.Count > 0)
                            PoE_Bot.Client.SendEmbed(embedAscendancyCont, lb.ChannelId);
            ...
            Log.W(new LogMessage(LogSeverity.Info, "Leaderboard Plugin", "Ticked Leaderboard"));
        }
    }
