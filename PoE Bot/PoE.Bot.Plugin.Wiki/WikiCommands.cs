            ...
            using (var httpClient = new HttpClient())
            {
                ...
                if (jObj.Count > 0)
                {
                    if (jObj.Count > 1)
                    {
                        ...
                        builder.Title = "Oops! More than one item was returned.";
                        builder.Description = "Sorry about that, below are the direct links to the Wiki for the items. Hope it helps!";
                        builder.Color = Color.Magenta;
                        builder.WithCurrentTimestamp();

                        for (int i = 0; i < jObj.Count; i++)
                        {
                            results.AppendLine(jsonSearchArray[1][i].ToString() + " " + jsonSearchArray[3][i].ToString());
                        }

                        builder.AddField("Search results:", results.ToString(), true);

                        return builder;
                    }
                    else
                    {
                        string itemName = jsonSearchArray[1].First.ToString();
                        string itemURL = jsonSearchArray[3].First.ToString();
